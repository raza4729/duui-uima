...
def get_batch(data, batch_size, unk_replace=0.0):
    data, data_size = data
    ...
    batch.update(
        {
            key: field[index, : 2 * max_length - 1]
            for key, field in data.items()
...
def get_bucketed_batch(data, batch_size, unk_replace=0.0):
    ...
    buckets_scale = [
        sum(bucket_sizes[: i + 1]) / total_size for i in range(len(bucket_sizes))
    ]
    ...
    bucket_id = min(
        [i for i in range(len(buckets_scale)) if buckets_scale[i] > random_number]
    )
...
def iterate_batch(data, batch_size, unk_replace=0.0, shuffle=False):
    data, data_size = data
    ...
    for start_idx in range(0, data_size, batch_size):
        ...
        batch.update(
            {
                key: field[excerpt, :batch_length]
                for key, field in data.items()
                if key not in exclude_keys
            }
        )
        batch.update(
            {
                key: field[excerpt, : 2 * batch_length - 1]
                for key, field in data.items()
                if key in stack_keys
            }
        )
...
def iterate_bucketed_batch(data, batch_size, unk_replace=0.0, shuffle=False):
    ...
    for bucket_id in bucket_indices:
        ...
        if unk_replace:
            ...
            noise = (
                single.new_empty(bucket_size, bucket_length)
                .bernoulli_(unk_replace)
                .long()
            )
            words = words * (ones - single * noise)
...
def iterate_data(data, batch_size, bucketed=False, unk_replace=0.0, shuffle=False):
    if bucketed:
        return iterate_bucketed_batch(
            data, batch_size, unk_replace == unk_replace, shuffle=shuffle
        )
    else:
